...
int main(int argc, char **argv) {
  ...
  cusparseDnVecDescr_t tempVecDescriptor;
  cusparseDnVecDescr_t tmpVecDescriptor;
  double flops = 0;
  ...
  cusparseCheck(cusparseCreateCsr(&Adescriptor,dimX,dimX,nzv,ARowPtr,AColIndx,
                                    A,CUSPARSE_INDEX_32I,CUSPARSE_INDEX_32I,CUSPARSE_INDEX_BASE_ZERO,CUDA_R_64F));
  cusparseCheck(cusparseCreateDnVec(&tempVecDescriptor, dimX,temp,CUDA_R_64F));
  cusparseCheck(cusparseCreateDnVec(&tmpVecDescriptor, dimX,tmp,CUDA_R_64F));
  ...
  cusparseCheck(cusparseSpMV_bufferSize(cusparseHandle, CUSPARSE_OPERATION_NON_TRANSPOSE, &one, Adescriptor, tempVecDescriptor, &zero,
                                          tmpVecDescriptor, CUDA_R_64F, CUSPARSE_SPMV_ALG_DEFAULT, &bufferSize));
  ...
  for (int it = 0; it < nsteps; ++it) {
    ...
    //@@ tmp = 1 * A * temp + 0 * tmp
    flops += nzv * 2;
    cusparseCheck(cusparseSpMV(cusparseHandle, CUSPARSE_OPERATION_NON_TRANSPOSE, &one, Adescriptor, tempVecDescriptor,
                               &zero, tmpVecDescriptor, CUDA_R_64F, CUSPARSE_SPMV_ALG_DEFAULT, buffer));
  ...
  double time = cputimer_stop("computing time");
  printf("The FLOPS is : %.2f\n", flops/time);
  ...
  cusparseDestroyDnVec(tempVecDescriptor);
  cusparseDestroyDnVec(tmpVecDescriptor);
